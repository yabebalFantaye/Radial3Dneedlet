  ...
  do ii=0,nmap_pp(me)-1
     ...
     if (i >= nshell) then !set by hand the map at the 0th shell to be 0
        ...
        do jj=0,nnmax
           ...
           !compute sin(pi*n*r)/r; if pi*n*r ~ r use sin(x)/x=1
           ! if (abs(rmid)>1e-10 .and. abs(PI*jj*rmid-rmid)>1e-10) then
           !    sinc = sinc(PI*jj*rmid)/rmid
           ! else
           !    sinc = 1d0
           ! endif

           alm_TGC = alm_TGC + argpi*jj*almn_TGC(:,:,:,jj)*sinc(argpi*jj*rmid)
        enddo
