program idl_ball2almn
  ...
  if  (me == 0) then
     ...
     do jj = 0,nnmax
        ...
        call fil_simp_points(rmin,rmax,y,nshell,1,rfili,wfili) !sin(xy)/xy 
        !alm_TGC = 0
        do ii=0,nshell-1  !remove monopole and dipole
           if  (feedback>2 .and. me == 0 .and. jj==0) print*, 'ball2almn: me, i, rmid,weight = ',me,ii, rfili(ii),wfili(ii)
           almn_TGC(1,2:lmax,0:lmax,jj) = almn_TGC(1,2:lmax,0:lmax,jj) + y*rfili(ii)**2*wfili(ii)*alm_TGC_r(ii,1,2:lmax,0:lmax) 
        enddo
        !almn_TGC(jj,:,:,:) = alm_TGC(:,:,:)
