...
!!!IDL extension to HEALPix [TESTVERSION]: Written by Frode K. Hansen, Nov. 2006
  ...
  ALLOCATE(map(0:npix-1),map1(0:npix-1),map2(0:npix-1), map_slet(0:npix-1,1:1+2*polar,0:nv,0:nj_pp(me)-1),alm(1:1+2*polar,0:lmax,0:lmax))
  ALLOCATE(arlm(1:1+2*polar,0:lmax,0:lmax))
  !allocate(arlmj(1:1+2*polar,0:lmax,0:lmax,0:nj_pp(me)-1))
  ...
  if (me==0)   allocate(arlm_all(1:1+2*polar,0:lmax,0:lmax))
  ...
  do in=0,nmax
     ...
     do j=first_j,last_j
        ...
        do iv=0,nv-1
           vval= dble(iv)
           map = map + map_slet(:,1,iv, j-first_j)*snrv(in, vval,j,bb,nv)
        enddo
  ...
  deALLOCATE( map1,map2, w8ring_map)
  deALLOCATE(map, map_slet)
  deallocate(arlm_all)
  ...
  if ( me.eq.0) then 
     ...
     do ir=0,nshell-1  !the zero r is set to zero by hand
        ...
        arlm = 0
        do in=0,nmax
           temp = argpi*in*sinc(argpi*in*rmid)
        ...
        if ( me.eq.0) call alm2map(nside_out,lmax,lmax,arlm,map_ball(:,1,ir))
