...
pro beta2ball_jmap,betamap,map,lmax=lmax_in,nmax=nmax_in,cl=cl_in,nj=nj,nv=nv,jint=jint,bb=bb,$
              ...
              nproc=nproc_in,rmin=rmin,rmax=rmax,nrsample=nrsample_in,almr=almr,outmap=outmap,almn=almn,$
                   maprj=maprj,outbeta=outbeta,norun=norun
...
fnamepar = dir+'params_beta2ball.unf'
  ...
  str='cd '+direxe+'; mpirun -np '+strtrim(string(nproc),2)+' -hostfile $HOME/mpihosts --prefix $MPI_HOME  ./idl_beta2ball_jmap '+fnamepar
  print, '************************* spawning **********************'
  print, '** ', str, ' **'
  print, '*********************************************************'
  ...
  tmpmap=fltarr(npix,nrpix)
  maprj=fltarr(npix,nrpix,nj)

  for j = 0,nj-1 do begin
     openr,unit,dir+'map_ball_beta_'+strn(j+1)+'.unf',/f77       
     readu,unit,tmpmap
     maprj[*,*,j]=tmpmap
     close,unit
  endfor
